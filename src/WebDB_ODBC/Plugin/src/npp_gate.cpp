...
#ifdef XP_MACOSX
  NPBool suppCoreGraphics = false;
  if (NPN_GetValue(instance, NPNVsupportsCoreGraphicsBool, &suppCoreGraphics) == NPERR_NO_ERROR && suppCoreGraphics) {
    NPN_SetValue(instance, NPPVpluginDrawingModel, (void*)NPDrawingModelCoreGraphics);
  } else {
    //CoreGraphics drawing model not supported, can't create a plugin instance.
    return NPERR_INCOMPATIBLE_VERSION_ERROR;
  }

 // select the Cocoa event model
  NPBool suppCocoaEvents = false;
  if (NPN_GetValue(instance, NPNVsupportsCocoaBool, &suppCocoaEvents) == NPERR_NO_ERROR && suppCocoaEvents) {
    NPN_SetValue(instance, NPPVpluginEventModel, (void*)NPEventModelCocoa);
  } else {
    //Cocoa event model not supported, can't create a plugin instance.
    return NPERR_INCOMPATIBLE_VERSION_ERROR;
  }
#else
...
#ifdef CHROME
    *((char **)value) = (char*)"OpenLink HTML5 WebDb-to-ODBC (Bridge) Plugin ver:1.0.8 for Chrome";
#else
    *((char **)value) = (char*)"OpenLink HTML5 WebDb-to-ODBC (Bridge) Plugin ver:1.0.8";
#endif
    break;
